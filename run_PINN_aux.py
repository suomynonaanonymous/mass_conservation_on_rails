...
from configs import *
from metrics import *
from models import PINN_backbone
...
# Extract N for div reduction step
...
for run in range(NUM_RUNS):
    ...
    # NOTE: The aux model requires an additional input dim
    model = PINN_backbone(input_dim = 3).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr = PINN_LR)

    train_losses_RMSE_over_epochs = torch.zeros(MAX_NUM_EPOCHS)
    train_losses_DIV_over_epochs = torch.zeros(MAX_NUM_EPOCHS)
    train_losses_PINN_COMBINED_over_epochs = torch.zeros(MAX_NUM_EPOCHS)
    ...
    for epoch in range(MAX_NUM_EPOCHS):
        ...
        train_losses_DIV_over_batches = 0.0
        train_losses_PINN_COMBINED_over_batched = 0.0
        ...
        for x_batch, y_batch in dataloader:
            ...
            batch_mse = torch.mean((y_pred_batch - y_batch) ** 2)
            ...
            batch_divergence_loss = compute_divergence_field(y_pred_batch, x_batch).abs().mean()
            
            # PINN LOSS
            # Compute loss as weighted sum: (RMSE for same units as data) + divergence loss
            loss = (1 - W_PINN_DIV_WEIGHT) * batch_mse + W_PINN_DIV_WEIGHT * batch_divergence_loss

            loss.backward()
            optimizer.step()

            # Add losses to the epoch loss (over batches)
            train_losses_RMSE_over_batches += torch.sqrt(batch_mse).item()
            train_losses_DIV_over_batches += batch_divergence_loss.item()
            train_losses_PINN_COMBINED_over_batched += loss.item()
        ...
        # NOTE: since we need to 3 dimensional inpute we sample N inputs from test 
        # NOTE: Not using labels
        idx = torch.randperm(N_test, device = x_test.device)[:N_PINN_div_reduction]          # random unique rows
        x_domain_random = x_test.index_select(0, idx)

        # full domain, permutation invariant
        x_domain_random = x_domain_random.to(device).requires_grad_()
        y_pred_domain = model(x_domain_random)
        ...
        domain_div_loss =  W_PINN_DIV_WEIGHT * compute_divergence_field(y_pred_domain, x_domain_random).abs().mean()
        ...
        avg_train_loss_DIV_for_epoch = train_losses_DIV_over_batches / len(dataloader)
        avg_train_loss_PINN_COMBINED_for_epoch = train_losses_PINN_COMBINED_over_batched / len(dataloader)

        # print("Epoch RMSE loss:", avg_train_loss_RMSE_for_epoch)
        # print("Epoch DIV loss:", avg_train_loss_DIV_for_epoch)
        # print("Epoch PINN loss:", avg_train_loss_PINN_COMBINED_for_epoch)

        train_losses_RMSE_over_epochs[epoch] = avg_train_loss_RMSE_for_epoch
        train_losses_DIV_over_epochs[epoch] = avg_train_loss_DIV_for_epoch
        train_losses_PINN_COMBINED_over_epochs[epoch] = avg_train_loss_PINN_COMBINED_for_epoch

        # Early stopping check
        if avg_train_loss_PINN_COMBINED_for_epoch < best_epoch_loss:
            best_epoch_loss = avg_train_loss_PINN_COMBINED_for_epoch
        ...
        if epoch % PRINT_FREQUENCY == 0 or epoch == MAX_NUM_EPOCHS - 1:
            print(f"Run {run} - {model_name} - Epoch {epoch} | Loss (Train PINN comb.): {avg_train_loss_PINN_COMBINED_for_epoch:.3f} - Loss (Train RMSE): {avg_train_loss_RMSE_for_epoch:.3f}")
    ...
    torch.save({
        ...
        "train_DIV": train_losses_DIV_over_epochs,
        "train_PINN_COMBINED": train_losses_PINN_COMBINED_over_epochs,
    ...
    trained_model = PINN_backbone(input_dim = 3).to(device)
