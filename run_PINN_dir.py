...
for run in range(NUM_RUNS):
    ...
    for epoch in range(MAX_NUM_EPOCHS):
        ...
        train_losses_PINN_COMBINED_over_batches = 0.0
        ...
        for x_batch, y_batch in dataloader:
            ...
            # Now calculate MSE first for consistency
            batch_MSE_loss = torch.mean((y_pred_batch - y_batch) ** 2)
            ...
            batch_DIR_loss = 1.0 - cos.mean() # 0 = same direction, 2 = opposite
            ...
            batch_DIV_loss = compute_divergence_field(y_pred_batch, x_batch).abs().mean()
            
            # PINN LOSS
            # Compute loss as weighted sum: (RMSE for same units as data) + divergence loss
            loss = (1 - W_PINN_DIV_WEIGHT / 2 - W_DIRECTIONAL_GUIDANCE / 2) * batch_MSE_loss + W_PINN_DIV_WEIGHT / 2 * batch_DIV_loss + W_DIRECTIONAL_GUIDANCE / 2 * batch_DIR_loss

            loss.backward()
            optimizer.step()

            # Add losses to the epoch loss (over batches)
            train_losses_RMSE_over_batches += torch.sqrt(batch_MSE_loss).item()
            train_losses_DIR_over_batches += batch_DIR_loss.item()
            train_losses_DIV_over_batches += batch_DIV_loss.item()
            train_losses_PINN_COMBINED_over_batches += loss.item()
        ...
        x_domain_random = torch.rand(N_PINN_div_reduction, 2)
        ...
        avg_train_loss_PINN_COMBINED_for_epoch = train_losses_PINN_COMBINED_over_batches / len(dataloader)
    ...
    trained_model = PINN_backbone().to(device)
